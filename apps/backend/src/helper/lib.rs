...
use lib::microservice::{EmailClient, StageStatus, StorageClient, VideoEditFlags};
...
//
// Deserialize is hand-written (see below) rather than derived because
// Firebase RTDB can send JSON `null` for fields that should be
// `HashMap::new()` / `false`. A `deserialize_with` attribute would do
// the job, but ts-rs parses serde attributes on `#[derive(TS)]` structs
// and emits a warning for every `deserialize_with` it encounters. Keeping
// the null-fallback logic in a manual impl on a wire struct keeps ts-rs
// happy and the exported TS type clean.
...
impl<'de> Deserialize<'de> for User {
    fn deserialize<D: serde::Deserializer<'de>>(deserializer: D) -> Result<Self, D::Error> {
        ...
        Ok(User {
            uid: w.uid,
            jobs: w.jobs.unwrap_or_default(),
            administrator: w.administrator.unwrap_or_default(),
        })
    }
}
...
pub enum JobStatus {
    ...
    Processing {
        #[ts(type = "string")]
        stage: lib::microservice::StageId,
        value: String,
    },
...
impl JobStatus {
    ...
    pub fn processing(stage: lib::microservice::StageId) -> Self {
        Self::Processing {
