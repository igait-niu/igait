...
use lib::microservice::{
    check_env, ClaimResult, EmailClient, EmailTemplates, FinalizeQueueItem, FirebaseRtdb,
    JobStatus, ProcessingResult, QueueOps, StageId, StageStatus, StorageClient,
...
impl FinalizeStageWorker {
    async fn process(&self, job: &FinalizeQueueItem) -> ProcessingResult {
        ...
        let result = if let Some(is_asd) = prediction_class {
            ...
            // Finalize owns the terminal state writes — there is no
            // orchestrator anymore.
            let prediction = if is_asd { 1.0 } else { 0.0 };
            self.update_stage_status(&job.job_id, finalize_id, StageStatus::Complete)
                .await;
            self.update_job_status(&job.job_id, JobStatus::complete(prediction, is_asd))
                .await;
            ...
            // Finalize itself succeeded (email delivered); the pipeline failed
            // upstream. Mark finalize Complete and the job overall Error so the
            // frontend transitions out of "Processing: Finalize".
            self.update_stage_status(&job.job_id, finalize_id, StageStatus::Complete)
                .await;
            self.update_job_status(&job.job_id, JobStatus::error(error_msg.clone()))
                .await;
...
async fn main() -> Result<()> {
    ...
    println!("Starting finalize worker (polling queue)");
    ...
    loop {
        match worker.queue_ops.claim_finalize_job().await {
            ...
            ClaimResult::Error(e) => {
                eprintln!("Claim error: {} — backing off", e);
                tokio::time::sleep(poll_interval).await;
            }
        }
    }
