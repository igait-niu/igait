...
(`backend`) plus all five pipeline stages (`media-conversion`,
`pose-estimation`, `cycle-detection`, `prediction`, `finalize`).

## Why this is separate from `infra/docker/frontend/`
...
one-line change in `lib` re-cook zero dependencies.
...
```sh
# from repo root
docker build -f infra/docker/workspace/Dockerfile --target backend -t backend .
```
