...
own `run_finalize_worker_mode()` in `apps/stages/finalize/src/main.rs`
...
`apps/shared/src/microservice/worker.rs`.
...
Jobs orchestrator loop in `apps/backend/src/helper/orchestrator.rs`; anything
